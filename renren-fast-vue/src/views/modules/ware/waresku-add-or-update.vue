  ...
  >
    ...
    >
      <el-form-item  label="商品" prop="skuName">
            <el-select style="width:300px" v-model="dataForm.skuName" placeholder="请选择商品" clearable>
              <el-option :label="s.skuName" :value="s.skuName" v-for="s in skuList" :key="s.id"></el-option>
            </el-select>
          </el-form-item>
      <el-form-item label="仓库" prop="wareId">
        <el-select style="width:300px" v-model="dataForm.wareId" placeholder="请选择仓库" clearable>
...
export default {
  data() {
    return {
      ...
      dataRule: {
        skuName: [{ required: true, message: "商品名不能为空", trigger: "blur" }],
        wareId: [
  ...
  methods: {
    ...
    getSkus() {
      this.$http({
        url: this.$http.adornUrl("/product/skuinfo/listsimple"),
        method: "get",
        params: this.$http.adornParams()
      }).then(({ data }) => {
    ...
    dataFormSubmit() {
      this.$refs["dataForm"].validate(valid => {
        this.skuList.forEach(sku => {
          if(sku.skuName == this.dataForm.skuName){
            this.dataForm.skuId=sku.skuId;
            return;
          }
        });
        if (valid) {
